...
void E30TTLUART_Resetting()//模块指令复位
{
...
void E30TTLUART_Config(uint16_t E30TTLUART_Address,uint8_t E30TTLUART_Channel, FunctionalState isAppointTargetSend)    //配置时波特率9600，模块工作波特率配置为115200
{
    uint8_t count;
...
void E30TTLUART_TransparentSendBytesData(uint8_t *bytesBuf, uint16_t bytesBufLength)//发送函数，对过长的函数截断
{
...
void E30TTLUART_Point2PointSendBytesData(uint16_t address,uint8_t channel,uint8_t* bytesBuf, uint16_t bytesBufLength)//点对点发送，需要地址及信道参数
{
