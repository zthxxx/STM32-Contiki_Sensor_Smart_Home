...
void RC522_GPIO_Config(void)
{
    ...
    GPIO_Init(RC522_NSS_GPIO_Port, &GPIO_InitStructure);
    
    GPIO_InitStructure.GPIO_Pin = RC522_RST_GPIO_Pin;
    GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP;
    GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
...
u8 ReadRawRC(unsigned char Address)
{
	...
	SPI2_WriteReadOneByte(ucAddr);
	ucResult=SPI2_WriteReadOneByte(0x00);
	SET_SPI_CS;
   return ucResult;
}
